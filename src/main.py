...
async def root():
    return {"message": "LLM Router v2.0 is running. Access /health or /healthz for status."}
...
async def prometheus_metrics():
    ...
    lines.append("# TYPE llm_router_circuit_breaker_open gauge")
    lines.append("# HELP llm_router_circuit_breaker_tripped_total Total times circuit breaker tripped")
    lines.append("# TYPE llm_router_circuit_breaker_tripped_total counter")
    for p, cb in cb_status.items():
        is_open_val = 1 if cb["state"] == "OPEN" else 0
        lines.append(f'llm_router_circuit_breaker_open{{provider="{p}"}} {is_open_val}')
        lines.append(f'llm_router_circuit_breaker_tripped_total{{provider="{p}"}} {cb["total_tripped"]}')
